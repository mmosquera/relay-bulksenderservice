...
namespace Relay.BulkSenderService.Classes
{
    public class ReportBuffer
    {
        ...
        private Dictionary<int, DBStatusDto> currentBuffer;
        ...
        public ReportBuffer(IConfiguration configuration, IUserConfiguration userConfiguration, DateTime date, int range, int offset)
        {
        ...
        private void CreateBufferFiles(DateTime from)
        {
            while (from < end)
            {
                ...
                var reportAux = new Dictionary<int, DBStatusDto>();
        ...
        private Dictionary<int, DBStatusDto> LoadBufferFromDisk(string fileName)
        {
            try
            {
